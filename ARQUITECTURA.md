...
- Se teclean seis conceptos por año —**ingresos brutos, patrimonio bruto, impuesto a cargo, retenciones, saldo final y justificación patrimonial**— y se guardan en `cifras` (sufijo `a` = año anterior, `b` = el del período). Son territorio de la plataforma: reimportar el Sheet no los toca, y quedan para el año siguiente y para reenviar el correo sin volver a teclearlos.
...
- **La justificación patrimonial también**: `ingresos del período − incremento patrimonial`, donde el incremento es `patrimonio bruto del período − patrimonio bruto del año anterior`. Por eso **solo existe en la columna del período**: calcularla para el año anterior pediría el patrimonio de un tercer año que el cuadro no tiene. Lleva en el correo un **asterisco con la explicación** (`NOTA_JUSTIFICACION`) —cero significa crecimiento respaldado por los ingresos, negativo significa valor pendiente de justificar—, y la nota solo sale si el renglón trae número.
